                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNYL ought to be enough for anybody." - Bill Gates, 2017
    ui->zNYLpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNYLSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNYL->setEnabled(false);
        ui->pushButtonMintzNYL->setToolTip(tr("zNYL is currently disabled due to maintenance."));

        ui->pushButtonSpendzNYL->setEnabled(false);
        ui->pushButtonSpendzNYL->setToolTip(tr("zNYL is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNYLpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNYL_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNYL is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNYL...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNYL in ") +
...
void PrivacyDialog::on_pushButtonSpendzNYL_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNYL is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNYL
        sendzNYL();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNYL
    sendzNYL();
}

void PrivacyDialog::on_pushButtonZNuyulControl_clicked()
{
    ZNuyulControlDialog* zNuyulControl = new ZNuyulControlDialog(this);
    zNuyulControl->setModel(walletModel);
    zNuyulControl->exec();
}

void PrivacyDialog::setZNuyulControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNuyulSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNYL()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Nuyul Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNYLpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNYL is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NYL </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNYL</b>";
    ...
    // use mints from zNuyul selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNuyulControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNuyulControlDialog::GetSelectedMints();
    }
    ...
    // Clear znuyul selector in case it was used
    ZNuyulControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNuyul Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Nuyul, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNuyul Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNYL </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNYL "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNYL "));
    ui->labelzNYLAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNYLSyncStatus->setVisible(fShow);
}
