...
#include "ui_znuyulcontroldialog.h"
...
std::list<std::string> ZNuyulControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNuyulControlDialog::listMints;

ZNuyulControlDialog::ZNuyulControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNuyulControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNuyulControlDialog::~ZNuyulControlDialog()
{
    delete ui;
}

void ZNuyulControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNuyulControlDialog::updateList()
{
...
void ZNuyulControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNuyulControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNuyul_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNuyulControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNuyulControlDialog::GetSelectedMints()
{
...
void ZNuyulControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
